...
export class UserService {
  ...
  create(request: UserRequest): Observable<UserResponse> {
    return this.http.post<BaseResponse<UserResponse>>(
      this.endpoint,
      request,
      AUTHORIZED
    ).pipe(
  ...
  update(id: string, request: UserRequest): Observable<UserResponse> {
    return this.http.put<BaseResponse<UserResponse>>(
      `${this.endpoint}/${id}`,
      request,
      AUTHORIZED
    ).pipe(
  ...
  updateActive(id: string): Observable<UserResponse> {
    return this.http.patch<BaseResponse<UserResponse>>(
  ...
  delete(id: string): Observable<UserResponse> {
    return this.http.delete<BaseResponse<UserResponse>>(
      `${this.endpoint}/${id}`,
      AUTHORIZED
    ).pipe(
      map(response => response.data)
