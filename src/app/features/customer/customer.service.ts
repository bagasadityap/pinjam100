...
export class CustomerService {
  ...
  getAll(): Observable<CustomerResponse[]> {
    return this.http
      .get<BaseResponse<CustomerResponse[]>>(`${this.endpoint}`, AUTHORIZED)
      .pipe(map((response) => response.data));
  }

  getPending(): Observable<CustomerResponse[]> {
    return this.http
      .get<BaseResponse<CustomerResponse[]>>(`${this.endpoint}/pending`, AUTHORIZED)
      .pipe(map((response) => response.data));
  }

  getVerifiedAndLimitIsNull(): Observable<CustomerResponse[]> {
    return this.http
      .get<BaseResponse<CustomerResponse[]>>(`${this.endpoint}/verified-limit-null`, AUTHORIZED)
      .pipe(map((response) => response.data));
  }

  getById(id: string): Observable<CustomerResponse> {
    return this.http
      .get<BaseResponse<CustomerResponse>>(`${this.endpoint}/${id}`, AUTHORIZED)
      .pipe(map((response) => response.data));
  }

  getDetailById(id: string): Observable<CustomerDetailResponse> {
    return this.http
      .get<BaseResponse<CustomerDetailResponse>>(`${this.endpoint}/${id}/detail`, AUTHORIZED)
      .pipe(map((response) => response.data));
  }

  verifyCustomer(customerId: string, status: string): Observable<CustomerDetailResponse> {
    return this.http
      .put<BaseResponse<CustomerDetailResponse>>(
